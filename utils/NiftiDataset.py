...
def resample_sitk_image(sitk_image, spacing=None, interpolator=None, fill_value=0):
    ...
    # orig_size = np.mat(orig_size) # added by me
    # orig_spacing = np.mat(orig_spacing) # added by me
    # orig_spacing = orig_spacing.resize(1, 3)
    # orig_size = orig_spacing.resize(1, 3)
    # print("orig_)size: " + str(orig_size.shape) + "\n") # added by me
    # print("orig_spacing: "+ str(orig_spacing.shape) + "\n") # added by me
    ...
    # new_spacing = np.mat(new_spacing) # added by me
    # print("new_spacing: " + str(new_spacing.shape)+ "\n") # added by me
...
class NifitDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = Normalization(image)  # set intensity 0-255 orginially uncommented
        ...
        if self.train:
            ...
            if Segmentation is False:
                label = Normalization(label)  # set intensity 0-255 originally uncommented
            castImageFilter.SetOutputPixelType(self.bit)
...
def Normalization(image):
    ...
    resacleFilter.SetOutputMaximum(255) # was 255
    ...
    def __call__(self, sample):
        ...
        while not contain_label:
            ...
            if Segmentation is False:
                ...
                threshold = sitk.BinaryThresholdImageFilter()
                threshold.SetLowerThreshold(1) # was 1
                threshold.SetUpperThreshold(255)
                threshold.SetInsideValue(1)
                threshold.SetOutsideValue(0)
