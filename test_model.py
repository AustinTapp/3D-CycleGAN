...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        setattr(self, 'netG' + opt.model_suffix, self.netG)  # original

    def set_input(self, input):
        # we need to use single_dataset mode
        self.real_A = input.to(self.device)  # the torch tensor patch in the inference function, original
        # self.image_paths = input['A_paths']

    def forward(self):
        self.fake_B = self.netG(self.real_A)  # original
